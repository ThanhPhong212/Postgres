...
// get a list of expert without investment funds
exports.getListExpert = async (req, res) => {
  try {
    ...
    if (userRole) {
      for (let i = 0; i < userRole.length; i++) {
        const fund = await InvestFund.findOne({ where: { expertId: userRole[i].userId } });
        if (!fund) data.push(userRole[i]);
      }
    }
    res.status(200).send({
      status: true,
